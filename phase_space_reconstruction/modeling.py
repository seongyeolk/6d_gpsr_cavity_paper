...
class PhaseSpaceReconstructionModel3D_palxfel_Simulation(torch.nn.Module):
    def __init__(self, lattice_SCM1, lattice_SCM2, diagnostic_SCM1, diagnostic_SCM2, beam):
        super(PhaseSpaceReconstructionModel3D_palxfel_Simulation, self).__init__()

        self.base_lattice1 = lattice_SCM1
        self.base_lattice2 = lattice_SCM2
        self.diagnostic_SCM1    = diagnostic_SCM1
        self.diagnostic_SCM2    = diagnostic_SCM2
        self.beam = deepcopy(beam)
    ...
    def track_and_observe_beam(self, beam, params, ids):
        ...
        # =================================================================================
        # SCM 1
        lattice_SCM1 = deepcopy(self.base_lattice1)
        lattice_SCM1.elements[ids[0]].K1.data      = params[:,0].unsqueeze(-1)
        lattice_SCM1.elements[ids[1]].K1.data      = params[:,4].unsqueeze(-1)
        lattice_SCM1.elements[ids[2]].PHI0.data    = params[:,1].unsqueeze(-1)
        lattice_SCM1.elements[ids[2]].VOLTAGE.data = params[:,3].unsqueeze(-1)
        lattice_SCM1.elements[ids[3]].K1.data      = params[:,5].unsqueeze(-1)
        lattice_SCM1.elements[ids[4]].K1.data      = params[:,6].unsqueeze(-1)
        
        # =================================================================================
        # SCM T
        lattice_SCM2 = deepcopy(self.base_lattice2)
        lattice_SCM2.elements[ids[0]].K1.data      = params[:,0].unsqueeze(-1)
        lattice_SCM2.elements[ids[1]].K1.data      = params[:,4].unsqueeze(-1)
        lattice_SCM2.elements[ids[2]].PHI0.data    = params[:,1].unsqueeze(-1)
        lattice_SCM2.elements[ids[2]].VOLTAGE.data = params[:,3].unsqueeze(-1)
        lattice_SCM2.elements[ids[3]].K1.data      = params[:,5].unsqueeze(-1)
        lattice_SCM2.elements[ids[4]].K1.data      = params[:,6].unsqueeze(-1)
        lattice_SCM2.elements[ids[5]].P0C.data     = params[:,2].unsqueeze(-1)
        lattice_SCM2.elements[ids[6]].P0C.data     = params[:,2].unsqueeze(-1)
        # ================================================================================
        

        # track through lattice
        initial_beam     = beam
        output_beam_SCM1 = lattice_SCM1(beam)
        output_beam_SCM2 = lattice_SCM2(beam)

        
        # analyze beam with diagnostic
        observations_SCM1 = self.diagnostic_SCM1(output_beam_SCM1) # SCM_H20 
        observations_SCM2 = self.diagnostic_SCM2(output_beam_SCM2) # SCM_H20 
        
        #observations = torch.stack((observations_SCM1, observations_SCM2))
        
    
        return observations_SCM1, observations_SCM2, output_beam_SCM1, output_beam_SCM2

    def forward(self, params, ids):
        proposal_beam = self.beam()

        # track beam
        observations_SCM1, observations_SCM2, output_beam_SCM1, output_beam_SCM2 = self.track_and_observe_beam(
            proposal_beam, params, ids
        )
        ...
        return observations_SCM1, observations_SCM2, output_beam_SCM1, output_beam_SCM2, entropy, cov
    


class PhaseSpaceReconstructionModel3D_palxfel_Experiment(torch.nn.Module):
    def __init__(self, lattice_SCM1, lattice_SCM2, diagnostic_SCM1, diagnostic_SCM2, beam):
        super(PhaseSpaceReconstructionModel3D_palxfel_Experiment, self).__init__()
...
class ImageDataset3D_EXP(Dataset):
    def __init__(self, params, images1, images2):
        self.params = params
    ...
    def __getitem__(self, idx):
        return self.params[idx], self.images1[idx], self.images2[idx]
    

class OutputDataset3D(Dataset):
    def __init__(self, params, output_beam, sigz):
        self.params = params
    ...
    def __getitem__(self, idx):
        return self.params[idx], self.output_beam[idx], self.sigz[idx]
